...
public class Assets {
    ...
    public static void queueLoading() {
        manager.load("skins/miniClip.pack", TextureAtlas.class);
    }

    //In here we'll create our skin, so we only have to create it once.
    public static void setMenuSkin() {
        if (menuSkin == null)
            menuSkin = new Skin(Gdx.files.internal("skins/miniClip.json"),
                    manager.get("skins/miniClip.pack", TextureAtlas.class));
    }
